...
- Imprimir en consola el array resultantante.*/
/*localStorage.setItem("personas",personasJSON);
let datos=localStorage.getItem("personas",personasJSON);
console.log(JSON.parse (datos))*/

let data = JSON.parse(personasJSON );


let array2=data.filter(p=>p.edad>18);
console.log(array2);
...
let datos=document.getElementById("datos")
 ...
 array2.forEach(persona=>{
    let datosPersona=document.createElement("div");
    datosPersona.innerHTML=`<p>Nombre:${persona.nombre[0]},Edad:${persona.edad}</p>`;
    datos.appendChild(datosPersona);
 });
